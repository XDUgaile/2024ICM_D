...
X=xlsread('相关关系.xlsx','A2:A49');
Y=xlsread('相关关系.xlsx','B2:B49');
...
figure;  % 新建图形窗口
surf(20+Udata*160,74+Vdata*2,reshape(Ccdf_Frank,size(Udata)));  % 绘制二元Frank-Copula分布函数图
xlabel('Precipitation(mm)');  % 为X轴加标签
...
figure;  % 新建图形窗口
contour(20+Udata*160,74+Vdata*2,Ccdf_Frank, 'LineWidth', 2);  % 绘制等值线图
xlabel('Precipitation(mm)');  % 为X轴加标签
ylabel('Water level(m)');  % 为Y轴加标签
